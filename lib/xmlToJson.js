...
function traverse(xml, attributeMode, attributesData) {
  ...
  while ((currentLevelTags = tagFinder.exec(xml)) !== null) {
    ...
    const [closingTagIndex, closingTag] = findClosingIndex(input, finishTag, tagLength);
    finishTag = closingTag;
...
function findClosingIndex(searchString, tag, start) {
  const openinTag = tag.replace('</', '<').replace('>', '').trim();
  const tagName = tag.replace('</', '').replace('>', '').trim();
  const closingTag = searchString.slice(start).match(new RegExp(`<\\/\\s*${tagName}\\s*>`, `g`))[0];

  let closingIndex = searchString.indexOf(closingTag, start);
  let openingIndex = searchString.indexOf(openinTag, start);

  if (closingIndex < openingIndex) {
    return [closingIndex, closingTag];
  }
  ...
  while (closingIndex > 0) {
    const tempIndex = searchString.indexOf(closingTag, closingIndex + 1);
    if (tempIndex > 0) {
